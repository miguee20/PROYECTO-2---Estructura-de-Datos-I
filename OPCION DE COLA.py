...
class VentanaPrincipal(QWidget):
    def __init__(self):
        ...
        layout.addWidget(self.boton_cola, alignment=Qt.AlignmentFlag.AlignCenter)
...
class VentanaCola(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ejemplo interactivo de una Cola")
        self.setGeometry(100, 100, 500, 400)

        self.definicion = QLabel("<h2>Una cola es una estructura de datos lineal que sigue el principio de FIFO (First In, First Out).</h2>"
                                 "<h2>Esto significa que el primer elemento que se inserta es el primero en ser eliminado.</h2>"
                                 "<h2>Las operaciones básicas de una cola son:</h2>"
                                 "<ul>"
                                 "<li>enqueue: inserta un elemento al final de la cola.</li>"
                                 "<li>dequeue: elimina y devuelve el elemento al principio de la cola.</li>"
                                 "<li>peek: devuelve el elemento al principio de la cola sin eliminarlo.</li>"
                                 "</ul>"
                                 "<h2>Ingrese un valor en el campo de texto y haga clic en 'Insertar' para agregarlo a la cola.</h2>"
                                 "<h2>Haga clic en 'Eliminar' para eliminar el primer elemento de la cola.</h2>"
                                 "<h2>Puede utilizar 'Buscar' para verificar si un elemento está presente en la cola.</h2>")
    ...
    def insertar_valor(self):
        valor = self.input_valor.text()
        if valor:
            self.cola.enqueue(valor)
            mensaje = f"Se ha insertado el valor '{valor}' en la cola.\n"
            mensaje += f"Orden actual de la cola: {self.cola.items}"
            self.resultado.setText(mensaje)
            self.resultado.setStyleSheet("color: green; font-weight: bold;")
            self.input_valor.clear()
        else:
    ...
    def eliminar_valor(self):
        valor_eliminado = self.cola.dequeue()
        if valor_eliminado:
            mensaje = f"{valor_eliminado}\n"
            mensaje += f"Orden actual de la cola: {self.cola.items}"
            self.resultado.setText(mensaje)
            ...
            self.mostrar_mensaje_error("La cola está vacía")

    def buscar_valor(self):
        valor = self.input_valor.text()
        if valor:
            mensaje = self.cola.buscar(valor)
            self.resultado.setText(mensaje)
            if "encontrado" in mensaje:
                self.resultado.setStyleSheet("color: green; font-weight: bold;")
            else:
